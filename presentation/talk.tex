...
\begin{frame}{Models: Labelled Transition Systems (\ltss)}
    ...
    An LTS is a tuple $\modlts = \tup{\W, \cset{\R_a}_{a \in \ACT}, \V}$ where:
        \begin{itemize} 
            \item $\W$ is a non empty countable set of states \quad\quad $\bullet$ $\V:\PROP \to 2^{\W}$
...
%=============================================================
    ...
    A plan must be \orange{fail-proof}: each partial execution must be completed.
    ...
    A plan $\sigma$ is \orange{strongly executable} (SE) at $u \in \W$ iff for all partial execution of $\sigma$ from $u$, such an execution can be completed.
