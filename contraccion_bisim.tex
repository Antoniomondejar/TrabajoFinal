...
Ahora bien, recordemos la definición de $A_\modults$ y de $\rho_\modults$, introducida en \Cref{def:A_m}, y veamos el siguiente teorema 
...
\begin{demostracion}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            Ahora bien, como $w \in U$ entonces $[w] \subseteq U$. Luego $v \in U$.

            Entonces demostramos que $U = A_\modults(U)$.

            Ahora bien, notemos que siendo $X \subseteq \W$ entonces $X \subseteq A_\modults(X)$, pues como analizamos anteriormente 
...
Sin embargo consideremos el siguiente modelo (\Cref{fig:1st_proposal_original}):
...
Con esta propuesta de contracción ($\modults'$), obtendríamos el modelo (\Cref{fig:1st_proposal_contraction}):
...
Luego consideremos el siguiente modelo (\Cref{fig:2nd_proposal_original}):
...
Y su contracción por bisimulación con la propuesta mencionada (\Cref{fig:2nd_proposal_contraction}):
