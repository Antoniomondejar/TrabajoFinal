...
punteado asociado es bisimilar al modelo punteado asociado a su clase equivalencia y, a su vez, es de tamaño mínimo entre todos los modelos 
bisimilares.
...
del mismo, basado en el algoritmo presentado por Paige y Tarjan en \cite{Paige&TarjanContraction} para computar la \textit{partición relacional 
más gruesa}.
...
atómicas consideradas por el modelo. A partir de este análisis surge la siguiente propuesta de contracción, la cual modifica el 
...
\begin{demostracion}
    ...
    polinomial. Por otro lado, notemos que por \Cref{thm:model-checking-poly} model-checking para \KHilogic está en $\Poly$ por lo que 
...
es computable eficientemente. Sin embargo, una observación que surge al analizar esta propuesta es que la contracción definida difiere estructuralmente, en 
...
indistinguibilidad sobre planes de cada agente. Recordando las primeras propuestas analizadas en la sección anterior, notemos que no 
logramos encontrar una propuesta adecuada a partir de la autobisimulación máxima $A_\model$ que no modifique las componentes 
...
\begin{demostracion}
    ...
    $w_0$ y $w'$ en $[w]$, existe una \bml-bisimulación $Z$ tal que $(w_0,w') \in Z$. Pero notemos que por (zig) de \bml-bisimulación, 
    como $(w_0,v_0) \in \R_a$  entonces existe $v'$ tal que $(w',v') \in \R_a$ y, a su vez, 
    $(v_0,v') \in Z$, es decir, $v' \in [v]$, que es lo que queríamos demostrar.
\end{demostracion}
...
\bml-bisimulación en $\bigO(m*\log(n))$, donde $n$ son la cantidad de nodos del modelo y $m$ la cantidad de aristas del mismo. 
...
\KHilogic, pues el \Cref{thm:adequacy-1st-cont} establece que para cada \ults existe otro lógicamente equivalente en el que cada 
