...
de valuación $\V$ siempre es una autobisimulación. Entonces, sea $\modults$ un \ults nos gustaría definir su contracción por bisimulación a partir de $A_\modults$, 
...
se conservarían. Sin embargo, consideremos el siguiente ejemplo (\Cref{fig:1stproposaloriginal}):
...
\begin{figure}[h]
    ...
    \qquad\qquad\qquad\qquad\qquad\qquad\qquad
            $\begin{array}{ll}
                ...
                \{b\} \right\}
            \end{array}$
    \caption{$\modults$ a la izquierda y $\model'$, su contracción, a la derecha.}
...
$\modults',[w_1]$ no son bisimilares. Por lo que esta contracción por bisimulación no sería adecuada, ya que no produce 
un modelo bisimilar.
...
$[w], [v] \in \rho_\modults$ cuando para cada $w' \in [w]$ existe $v' \in [v]$ tal que $w'$ y $v'$ están relacionados en el modelo 
original. Luego consideremos el siguiente modelo (\Cref{fig:2ndproposaloriginal}):
...
$\modults,w_1$ y $\modults',[w_1]$ no son bisimilares. Por lo que esta propuesta tampoco es adecuada, ya que tampoco produce un modelo 
...
autobisimulación máxima $A_\modults$. Luego, es deseable que sea razonable en términos computacionales. Con esto en mente, presentamos 
...
\begin{demostracion}
    ...
    $\sexec^{\model}(\pi)$ y $\R_\pi$ para cada $\pi$ es realizable en tiempo polinomial. Luego $\modults'$ es computable en tiempo polinomial.
\end{demostracion}
...
logramos encontrar una propuestra adecuada a partir de la autobisimulación máxima $A_\modults$ que no modifique las componentes 
...
contracciones propuestas. En este primer ejemplo, consideraremos el siguiente modelo $\modults_1$ (\Cref{fig:1st-ex-cont}):
...
\end{minipage}
    ...
    \end{tikzpicture}
     ...
     %    \end{minipage}
     \end{tabular}
    \caption{$\model$ (izquierda), 1ra propuesta (derecha arriba) y 2da propuesta (derecha abajo).}
