...
\begin{demostracion}
    ...
    polinomial. Luego notemos que por \Cref{thm:model-checking-poly}, model-checking está en $\Poly$ por lo que es claro que computar 
...
Hemos presentado dos propuestas de contracción por bisimulación para \KHilogic para las cuáles su resultado esperado de correctitud y 
...
\KHilogic, pues el \Cref{thm:adequacy-1st-cont} establece que para cada \ults existe otro que es lógicamente equivalente en el que cada 
...
Continuando con la segunda propuesta de contracción, el primer detalle a considerar es que el modelo obtenido $\model_\bml$ no es minimal 
...
no produce necesariamente un modelo lógicamente equivalente. Por lo que el \Cref{thm:adequacy-2nd-cont} es un resultado 
...
de la \Cref{fig:2nd-ex-cont} en el cual $\S'_i = \{ \{a_a\},\{a_{ab}\},\{a_{ac}\},\{a_{ad}\},\{a_{aba}\},\{a_{aca}\},\{a_{ada}\}\}$, mientras 
que su contracción a partir de la segunda propuesta $\model_\bml$ se encuentra abajo a la derecha en la misma figura. Este es un claro 
ejemplo en el que la primera propuesta produce un modelo con una cantidad de aristas significativamente mayor a la del modelo original. 
...
óptima que logre minimizar la suma del tamaño de cada componente. Aún así, ambas propuestas proveen buenas aproximaciones a la solución
que no solo sirven en contextos de aplicaciones prácticas sino que también profundizan el entendimiento teórico del poder expresivo de 
\KHilogic y de la clase de modelos \ultss. 
