...
El ejemplo que constituye las bases teóricas en las que surge este trabajo es el de las lógicas epistémicas \cite{Hintikka1962,HandbookEpistemicLogic}. 
Estas permiten representar 
...
lloviendo en Río Cuarto'', donde el valor de verdad de la propiedad no solo depende de si efectivamente llueve en Río Cuarto sino 
...
La semántica para este tipo de lógicas está basada en la noción de \textit{mundos posibles} \cite{Kripke-possible-worlds}. 
Dada la información actual 
que posee un agente existe una cierta cantidad de mundos posibles que podrían representar el estado actual de los hechos. 
Luego, un agente \textit{sabe} que una propiedad vale en un mundo, si la misma es cierta en todos los mundos que considera que son posibles. 
...
Las propuestas clásicas de lógicas epistémicas consideran el conocimiento de los agentes en la forma de ``saber qué'' (\textit{knowing that}, \cite{Hintikka1962,HandbookEpistemicLogic}), 
...
Así es como surge la propuesta de \cite{ArecesFSV25,SaraviaPHD} de una Lógica de \textit{Knowing How} Basada en Incertidumbre (\KHilogic), 
...
aparecen nociones de equivalencia de modelos que consideran únicamente características estructurales de los mismos;
evitando la necesidad de objetos sintácticos, como lo son las fórmulas, para identificar modelos con el mismo comportamiento.
...
En el \Cref{ch:marco-teorico} presentamos la Lógica de \textit{Knowing How} Basada en Incertidumbre (\KHilogic). 
...
un análisis sobre las características de cada propuesta junto a ejemplos que evidencian la naturaleza de ambas.
