...
\begin{ejemplo}
    ...
    Luego, si consideramos la relación binaria $Z = \{(w,w) \mid \mathcal{P}(\PROP) \times \mathcal{P}(\PROP)\}$, podemos ver que $Z$ no es una 
    \KHilogic-bisimulación$^*$ entre $\model$ y $\model'$ por lo analizado en el párrafo anterior. Sin embargo, como $\{x\}$ \textbf{no} 
