...
\begin{frame}
  ...
  \begin{footnotesize}
    ...
    \begin{block}{\textbf{Definición (\KHilogic-bisimulación)}}
      Sean $\model, \model'$ dos \ultss, una relación binaria $Z \subseteq \W\times\W'$ es una \orange{\KHilogic-bisimulación} entre $\model$ y $\model'$ si y sólo si\pause
  \begin{description}
    \item[(Atom)] $\V(w) = \V'(w')$ para cada $(w,w')\in Z$. \pause
    \item[($\khi$-zig):] para cada $U \subseteq \W$ \emph{proposicionalmente} definible, si $U \ultsExecAgi T$ para $T \subseteq \W$, existe $T' \subseteq \W'$ tal que: 
    1) $Z(U) \ultsExecAgi T'$ y
    2) $T' \subseteq Z(T)$. \pause
    
    \item[($\khi$-zag):] % analogous to \textbf{$\khi$-Zig}.
    para cada $U' \subseteq \W'$ \emph{proposicionalmente} definible, si $U' \ultsExecAgi T'$ para $T' \subseteq \W'$, existe $T \subseteq \W$ tal que: 
    1) $Z^{-1}(U') \ultsExecAgi T$ y
    2) $T \subseteq Z^{-1}(T')$. \pause
    
    \item[($\A$-zig):] para cada $w\in\W$ existe $w'\in\W'$ tal que $(w,w')\in Z$. \pause
    
    \item[($\A$-zag):] para cada $w'\in\W'$ existe $w\in\W$ tal que $(w,w')\in Z$.
  \end{description}
\end{block}
\end{footnotesize}
\end{frame}
...
\begin{frame}
  ...
  \begin{small}
    ...
    \begin{teorema}[\gray{Areces et al. 2021}]
      Sean $\model$ y $\model'$ dos \ultss. Si existe $Z$ una \KHilogic-bisimulación entre ellos tal que $(w,w')\in Z$, entonces 
  ...
  Esta definición cuenta con algunos aspectos dignos de ser cuestionados. \pause
  \vspace{0.1cm}
  \begin{footnotesize}
...
\begin{frame}
  \frametitle{Redefiniendo \KHilogic-bisimulación}
\end{frame}
