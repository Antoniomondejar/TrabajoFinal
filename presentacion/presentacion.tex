...
\begin{frame}
  \frametitle{Lógica epistémica basada en habilidades}\pause
  \begin{figure}[h]
        % \hspace{2.3cm}
        \vspace{-0.5cm}
        \begin{tikzpicture}
    ...
    \end{figure}\pause
% \vspace{0.1cm}
    ¿Ana/Pedro sabe cómo llegar a Parque Horizonte dado que está en el Centro?
\end{frame}
...
% \vspace{0.1cm}
    \begin{itemize}
      \item \small $\kh_{Ana}$(Centro,Pq.Hor) {\color{darkgreen}es cierto}. \pause
      \item \small $\kh_{Pedro}$(Centro,Pq.Hor) \red{no es cierto}.
    \end{itemize}
\end{frame}
  ...
  \begin{tikzpicture}
      ...
      \onslide<2->{\node[state, yshift=-1cm] (p) {$p$};
          ...
          \node at ($(r)+(0,-0.5)$) {$[w_5]$};
            ...
            \onslide<3->>{\path (p) edge [bend left=40] node [above] {$a_{ab}$} (r);}
  \end{tikzpicture}
  \end{minipage}
  \end{figure}
  \hspace{0.5cm}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Introdujimos una segunda propuesta de contracción por \KHilogic-bisimulación, la cual está basada en la contracción clásica de 
