...
\begin{frame}
  ...
  En primera instancia, estudiamos el problema de determinar la existencia de una \KHilogic-bisimulación 
...
\begin{frame}
  ...
  Reduciremos polinomialmente $\DNFTAUT$ a $\CBisim$.
\end{frame}
...
\begin{frame}
  \frametitle{Estudio computacional de \KHilogic-bisimulación}
    Estudiamos la complejidad computacional de la ``verificación''. \pause La ``búsqueda'' parece costosa,  existe una cantidad exponencial 
    ...
    \begin{small}
      ...
      \begin{lema}\label{lema:bisim-existence}
        ...
        \end{center}
        es una \KHilogic-bisimulación entre $\model$ y $\model'$.
      \end{lema}\pause
    \end{small}

    Como consecuencia de este lema, decidir la existencia de una bisimulación se reduce a verificar si $\universalbis$ es una bisimulación.
...
\begin{frame}
  ...
  \begin{small}
    \begin{teorema}
      Sea $\model$ un \ults y sea $\fcont{\model}$ su contracción por \KHilogic-bisimulación, entonces existe $Z$ una \KHilogic-bisimulación 
...
\vspace{-0.7cm}
\begin{center}
 $\S_i = \left\{ \{ab\}, \{ac\}  \right\}$, $\S_i' = \left\{ \{a_{ab}\}, \{a_{ac}\} \right\}$ \\[0.3cm]
 $\model$ (izquierda), $\fcont{\model}$ (derecha arriba) y $\model_\bml$ (derecha abajo).
\end{center}
\end{footnotesize}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Establecimos que tanto verificar como decidir existencia de una bisimulación entre dos \ultss es $\coNPComplete$. A su vez, 
    extendimos este resultado al caso punteado de este problema.\pause
    \item Abordamos el problema de la minimización de modelos, para el cual propusimos dos contracciones por \KHilogic-bisimulación las 
