...
\begin{demostracion}
    ...
    que atestigua dicha situación \textbf{no} es aquella asignación que asigna \textit{false} a todas las variables proposicionales de $\varphi$. 
...
que la parte de \textit{búsqueda} propia de este problema se trivializa.
...
\begin{demostracion}
    ...
    y a $\tup{\modults,\modults'}$ con $\modults$ y $\modults'$ dos \ultss fijos no bisimilares en caso contrario, basta 
...
Notemos que por \Cref{thm:new-bisim-finite-equiv}, sabemos que es equivalente decidir $\modults,w \bisim \modults',w'$ y 
...
existe una \KHilogic-fórmula de tamaño polinomial que los distingue. Más aún, mencionaremos cómo \textsc{CounterexampleChecker} permite construir 
dicha fórmula.
...
Antes de continuar con la demostración, primero introduciremos una notación que nos será de utilidad para construir la fórmula deseada.
...
$\truthset{\model'}{\varphi_X} = \universalbis(X)$.
...
\begin{itemize}
    ...
    Luego, $\varphi = \E\varphi_{[v]}$ sirve para probar la propiedad deseada.
    ...
    $\V(u) = \V'(v')$, por lo que $(u,v') \in \universalbis$. Con lo que podemos concluir que $v' \in \universalbis(T)$.
...
De esta manera, además de estudiar las propiedades computacionales de la lógica, obtenemos una nueva aplicación de nuestros métodos a 
la hora de detectar las diferencias entre dos modelos de manera sintáctica.