...
entre dos modelos $\modults$ y $\modults'$ basta con verificar que $Z'$ es una \KHilogic-bisimulación.

\begin{demostracion}

    Notar que la dirección ($\leftarrow$) es directa. Demostraremos entonces ($\rightarrow$).

    Queremos ver que $Z'$ es una \KHilogic-bisimulación entre $\modults$ y $\modults'$,
...
\begin{demostracion}
    ...
    $\modults$ y $\modults'$ son \KHilogic-bisimilares si y sólo si para todo $x \in \{0,1\}^*$ con 
    $|x| \le p(|\modults|+|\modults'|)$,  $A(\modults,\modults',x) = 1$. Esencialmente, a los $x$ los 
    ...
    Proponemos entonces el siguiente algoritmo:

    \begin{algorithm}
        \caption{Verificador de contraejemplos}
        \begin{algorithmic}[1]
            \Function{Simulates}{$\model,\model',Z',U$}
                \ForAll{$\S_i$}
                    \ForAll{$\pi \in \S_i$}
            ...
            \Function{CheckBisimilarity}{$\modults,\modults',\tup{U,b}$} 
                \State Compute $Z'$
                ...
                \If{$b$}
                    ...
                    \Return 1
                    \State \Return \Call{Simulates}{$\model,\model',Z',U$}
                \Else
                    \State If there exists $s \in \rho_{\model'}$ such that $s \not\subseteq U$ and $s \cap U \neq \emptyset$ 
                    \Return 1
                    \State \Return \Call{Simulates}{$\model',\model,Z'^{-1},U$} \Comment{Notice the order of the parameters}
                \EndIf
            \EndFunction
        \end{algorithmic}
    \end{algorithm}

    Primero notemos que el algoritmo propuesto corre en tiempo polinomial en el tamaño de $\tup{\modults,\modults'}$.
    ...
    Veamos ahora que $\tup{\modults,\modults'} \in \KHiBisim$ si y sólo si
    la ejecución de CheckBisimilarity a partir de ($\modults,\modults',\tup{U,b}$) devuelve 1 para 
    cada $\tup{U,b}$ donde $U$ es subconjunto del dominio de alguno de ambos modelos y $b$ es un 
    bit que nos indica de qué dominio es subconjunto $U$.

    \begin{itemize}
        \item ($\rightarrow$)
        Sea $\tup{\modults,\modults'} \in \KHiBisim$ y $\tup{U,b}$, 
        veamos que la ejecución de CheckBisimilarity a partir de ($\modults,\modults',\tup{U,b}$) devuelve 1.

        Primero notemos que como $\tup{\modults,\modults'} \in \KHiBisim$, entonces $Z'$ es una bisimulación 
        entre $\modults$ y $\modults'$.
        ...
        \begin{itemize}
            \item $b = 1$
                Primero notemos que si $U$ no es proposicionalmente definible en $\modults$, entonces el algoritmo devuelve 1.
        ...
        existe $\tup{U,b}$ tal que la ejecución de CheckBisimilarity a partir de ($\modults,\modults',\tup{U,b}$) 
        devuelve 0.  
        
        Como $\tup{\modults,\modults'} \not \in \KHiBisim$, entonces sabemos que $Z'$ no es una bisimulación.
        ...
        \begin{itemize}
            ...
            Analicemos entonces la ejecución de CheckBisimilarity a partir de ($\modults,\modults',\tup{U,1}$).
            ...
            Finalmente, CheckBisimilarity($\modults,\modults',\tup{U,1}$) = 0.
...
\medskip\medskip
    ...
    Ahora queremos demostrar que $\varphi \in \DNFTAUT$ si y sólo si $\tup{\modults,\modults'} \in \KHiBisim$.
    \begin{itemize}

    \item $(\rightarrow)$ Sea $\varphi \in \DNFTAUT$, donde $\varphi = \varphi_1 \vee ... \vee \varphi_m$ y $p_1,...,p_n$ 
    son las variables proposicionales que aparecen en $\varphi$, veamos que $\tup{\modults,\modults'} \in \KHiBisim$.
    ...
    Veamos que $Z := \{(w,w) \mid w \in \W \}$ es una bisimulación entre $\modults$ y $\modults'$. Es fácil ver que 
    ...
    Notemos que cada plan de $\modults'$ está también en $\modults$, y más aún, las aristas con las etiquetas 
    ...
    Juntando los puntos mencionados, demostramos que $Z$ es una bisimulación entre $\modults$ y $\modults'$, 
    por lo que $\tup{\modults,\modults'} \in \KHiBisim$. 
    ...
    Lo cual demuestra que $\tup{\modults,\modults'} \not \in \KHiBisim$.
    \end{itemize}
