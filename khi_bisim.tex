...
Demostraremos la equivalencia via reducción de Karp entre $\KHiBisim$ y $\PKHiBisim$, es decir, demostraremos que 
...
A partir de la definición de $\varphi_s$, no es díficil notar que $\truthset{\model}{\varphi_s} = s$. Por otro lado, para $X \subseteq \W$
...
\begin{itemize}
    \item (A-zig) Si $\universalbis$ no satisface (A-zig), entonces existe $v\in\W$ para el cuál no existe $v'\in\W'$ tal que $(v,v')\in\universalbis$.
    ...
    se cumple que $s \cap U = \emptyset$ o $s\subseteq U$, y $U \ultsExecAgi T$ para los cuáles no existe $T'\subseteq\W'$ tal que 
    \begin{multicols}{2}
        \begin{itemize}
            \item $\universalbis(U) \ultsExecAgi T'$, 
            \item $T' \subseteq \universalbis(T)$.
    ...
    Pero esto es una contradicción, dado que iniciamos este caso suponiendo que no existía tal $T'$. Luego, podemos concluir que $\model',w'\not\models\varphi$.
...
como input y el conjunto $T$ definido en la línea \ref{line:T-reachable-states-counterexcheck} para el cuál no se encontró un testigo, 
