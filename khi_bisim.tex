...
\begin{demostracion}
    ...
    \begin{algorithm}[t]
        ...
        \begin{algorithmic}[1]
            \Function{NotSimulates}{$\model,\model',Z,U$}
                \ForAll{$\S_i$}
                    \ForAll{$\pi \in \S_i$}
                        ...
                        \State \textit{foundStrat} $\gets 0$
                            \ForAll{$\pi' \in \S_i'$}
                                \If{$U' \subseteq \sexec^{\model'}(\pi') \wedge \R'_{\pi'}(U') \subseteq T$}
                                    \State \textit{foundStrat} $\gets 1$
                                \EndIf         
                            \EndFor
                            \If{$\neg$ \textit{foundStrat}}
                                \Return 1
            ...
            \Function{CounterexampleChecker}{$\modults,\modults',Z,\tup{U,b}$} 
                ...
                \If{$b$}
                    \State If exists $s \in \rho_\model$ such that $s \not\subseteq U$ and $s \cap U \neq \emptyset$ 
                    \Return 0
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            ingresa al \textit{if} de la línea 4 e inicializa la variable \textit{foundStrat} en 0.
            ...
            el valor de \textit{foundStrat} permanece en 0 y \textsf{NotSimulates} devuelve 1 en la línea 11.
            ...
            Por lo que \textsf{NotSimulates} abandona el $for$ de la línea 8 con la variable \textit{foundStrat} valiendo 0, luego 
        ...
        \begin{itemize}
            \item $b = 1$
                ...
                entonces el algoritmo ingresa al \textit{if} de la línea 4 e inicializa \textit{foundStrat} en 0.
                ...
                el algoritmo al iterar sobre el plan $\pi'$, actualiza el valor de \textit{foundStrat} a 1.
...
\begin{definicion}\label{def:karp-reduction}
    ...
    con:
    \begin{itemize}
        \item $\W \eqdef \{p_i\mid i\in\{1,...,n\}\} \cup \{\varphi_i \mid i \in \{1,...,m\}\} \cup \{e\}$
        \item $\ACT \eqdef \{in_{\varphi_i} \mid i \in \{1,...,m\}\} \cup \{out_{\varphi_i} \mid i \in \{1,...,m\}\}$
        \item $\S \eqdef \{\{in_{\varphi_i}out_{\varphi_i}\} \mid i \in \{1,...,m\}\}$
