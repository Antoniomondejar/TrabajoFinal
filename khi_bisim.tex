\chapter{Estudio computacional de \KHilogic-bisimulación}\label{ch:khi-bisim}

Como mencionamos en el anterior capítulo, a la hora de definir una noción de bisimulación para una lógica modal, 
es deseable que la misma tenga una naturaleza puramente estructural, lo cual resulta de gran utilidad a la hora de 
definir un procedimiento que dados dos modelos y una relación binaria entre sus dominios pueda decidir 
si dicha relación es una bisimulación.

Esta naturaleza estructural que facilita verificar si una relación entre los dominios de dos modelos es una bisimulación,
también nos permite encontrar un procedimiento efectivo que decida si existe una bisimulación entre dos modelos.
Dicho problema de decisión es usualmente conocido en la literatura de las lógicas modales como comparación de modelos.
...
nos referiremos siempre a \ultss finitos, es decir, \ultss en los cuales cada una de sus componentes tiene tamaño finito.
...
Una vez definido matemáticamente el problema en cuestión, nos interesaría determinar los recursos computacionales necesarios 
para resolverlo.
...
\begin{demostracion}
    Para demostrar que $\CBisim \in \coNP$ debemos demostrar que su complemento está en la clase $\NP$. Para ello, basta con dar un 
    ...
    Esencialmente, a los $x$ los llamaremos \textit{contraejemplos} y sólo nos interesarán los que sean de la forma $\tup{U,b}$ con $U$ un 
    ...
    Veamos que la función \textsc{CounterexampleChecker} en el \Cref{alg:counter-example-checker} cumple con lo requerido.
    ...
    \begin{algorithm}[t]
        ...
        \begin{algorithmic}[1]
            \Function{NotSimulates}{$\model,\model',Z,U$}
                \ForAll{$\S_i$}
                    \ForAll{$\pi \in \S_i$}
                        \If{$U \subseteq \sexec^{\model}(\pi)$}\label{line:pi-se-in-U}
                        \State $T \gets \R_\pi(U)$\label{line:T-reachable-states-counterexcheck}
                        \State $U' \gets Z(U)$
                        \State $T' \gets Z(T)$ 
                        \State \textit{foundWitness} $\gets 0$
                            \ForAll{$\pi' \in \S_i'$}\label{line:for-strat-in-m'}
                                \If{$U' \subseteq \sexec^{\model'}(\pi') \wedge \R'_{\pi'}(U') \subseteq T'$}
                                    \State \textit{foundWitness} $\gets 1$
                                \EndIf         
                            \EndFor
                            \If{$\neg$ \textit{foundWitness}}
                                \Return 1\label{line:did-not-find-witness}
                            \EndIf
                        \EndIf
                    \EndFor
                \EndFor
                \State \Return 0\label{line:return-0}
            \EndFunction

            \vspace{1.5em}

            \Function{CounterexampleChecker}{$\modults,\modults',Z,\tup{U,b}$} 
                \State Check (Atom), if not \Return 1\label{line:Atom}
                \State Check (A-zig), if not \Return 1\label{line:A-zig}
                \State Check (A-zag), if not \Return 1\label{line:A-zag}
                \If{$b$}
                    \State If there exists $s \in \rho_\model$ such that $s \not\subseteq U$ and $s \cap U \neq \emptyset$\label{line:khi-zig-check} 
                    \Return 0
                    \State \Return \Call{NotSimulates}{$\model,\model',Z,U$}\label{line:not-simulates-exec}
                \Else
                    \State If there exists $s' \in \rho_{\model'}$ such that $s' \not\subseteq U$ and $s' \cap U \neq \emptyset$\label{line:khi-zag-check}
                    \Return 0
    ...
    Notemos que los chequeos de las líneas \ref{line:Atom}, \ref{line:A-zig}, \ref{line:A-zag}, \ref{line:khi-zig-check} y 
    \ref{line:khi-zag-check} son claramente realizables en tiempo polinomial. Por 
    otro lado, analizando la función \textsc{NotSimulates} podemos ver que los cómputos de $\R_\pi$ y $\sexec(\pi)$ 
    ...
    \textsc{CounterexampleChecker} corre en tiempo polinomial con respecto al tamaño de $\tup{\model,\model',Z}$.
    
    Ahora, queremos ver que $\tup{\modults,\modults',Z} \not\in \CBisim$ si y sólo si
    la ejecución de \textsc{CounterexampleChecker} a partir de ($\modults,\modults',Z,\tup{U,b}$) devuelve 1 para 
    algún $\tup{U,b}$ donde $U$ es un subconjunto del dominio de alguno de los \ults y $b$ es un 
    bit que nos indica de qué dominio es subconjunto $U$.

    \bm{$\Rightarrow)$} Queremos ver que siendo  
    $\tup{\modults,\modults',Z} \not \in \CBisim$ entonces
    existe $\tup{U,b}$ tal que la ejecución de \textsc{CounterexampleChecker} a partir de ($\modults,\modults',Z,\tup{U,b}$) 
    devuelve 1.
    ...
    \begin{itemize}
        \item (Atom) Si $Z$ no satisface (Atom) entonces el algoritmo lo identifica en la línea \ref{line:Atom} y devuelve 1, por lo 
        que cualquier $\tup{U,b}$ sirve como contraejemplo.
        \item (A-zig) Si $Z$ no satisface (A-zig) entonces el algoritmo lo identifica en la línea \ref{line:A-zig} y devuelve 1, 
        por lo que cualquier $\tup{U,b}$ sirve como contraejemplo.
        \item (A-zag) Similarmente, si $Z$ no satisface (A-zag) el algoritmo lo identifica en la línea \ref{line:A-zag} y,  
        ...
        Analicemos entonces la ejecución de \textsc{CounterexampleChecker} a partir de ($\modults,\modults',Z,\tup{U,1}$).

        Como $Z$ satisface (Atom), (A-zig) y (A-zag); y, a su vez, $U$ satisface que para cada $s \in \rho_\model$ se cumple 
        que $s \cap U = \emptyset$ o $s \subseteq U$, entonces se ejecuta la instrucción de la línea \ref{line:not-simulates-exec} y 
        el valor de retorno de \textsc{NotSimulates} determina el valor de retorno de \textsc{CounterexampleChecker}.
        ...
        Esto nos dice que al considerar la ejecución de \textsc{NotSimulates}, cuando se itera sobre el plan $\pi$, el algoritmo 
        ingresa al \textit{if} de la línea \ref{line:pi-se-in-U} e inicializa la variable \textit{foundWitness} en 0.
        ...
        el valor de \textit{foundWitness} permanece en 0 y \textsc{NotSimulates} devuelve 1 en la línea \ref{line:did-not-find-witness}.
        ...
        Por lo que \textsc{NotSimulates} abandona el $for$ de la línea \ref{line:for-strat-in-m'} con la variable 
        \textit{foundWitness} valiendo 0, luego devuelve 1 en la línea \ref{line:did-not-find-witness}.

        Finalmente, \textsc{CounterexampleChecker}($\modults,\modults',Z,\tup{U,1}$) = 1.

        \item ($\khi$-zag) Análogo al caso recién analizado, notando que la forma en la que se llama a \textsc{NotSimulates}
        considera el caso ($\khi$-zag).
    \end{itemize}

    \bm{$\Leftarrow)$} Queremos ver que si existe $\tup{U,b}$ tal que la ejecución de \textsc{CounterexampleChecker} a partir de 
    $(\model,\model',Z,\tup{U,b})$ devuelve 1 entonces $\tup{\model,\model',Z} \not\in \CBisim$. 
    Demostraremos este caso a partir de su contrarrecíproca. Luego, 
    sea $\tup{\modults,\modults',Z} \in \CBisim$ queremos ver que para todo $\tup{U,b}$ la ejecución de \textsc{CounterexampleChecker} 
    ...
    Luego, viendo la ejecución de \textsc{CounterexampleChecker}, los chequeos de (Atom), (A-zig) y (A-zag) darán 1 como resultado.
    
    Analicemos ahora qué sucede para cada posible valor de $b$:

    \begin{itemize}
        \item $b = 1$.
            Primero, notemos que si $U$ no satisface que para cada $s \in \rho_\model$ se cumple que $s \cap U = \emptyset$ 
            o $s \subseteq U$, entonces el algoritmo devuelve 0 en la línea \ref{line:khi-zig-check}.

            Supongamos entonces $U$ cumple con lo mencionado. Luego, se ejecuta \textsc{NotSimulates} en la línea \ref{line:not-simulates-exec}. 
            
            Sea $\pi \in \S_i$ tal que $U \subseteq \sexec^{\model}(\pi)$, 
            entonces el algoritmo ingresa al \textit{if} de la línea \ref{line:pi-se-in-U} e inicializa \textit{foundWitness} en 0.            
            ...
            Como este análisis vale para todo $\pi \in \S_i$, esto demuestra que \textsc{NotSimulates} finaliza su ejecución en la 
            línea \ref{line:return-0} donde devuelve 0. Por lo que \textsc{CounterexampleChecker} devuelve 0 a partir de 
            ($\modults,\modults',\tup{U,b}$).
        \item $b = 0$.
...
Sin embargo, a la hora de determinar que un problema se encuentra en una clase de complejidad como $\coNP$, también es deseable 
...
Con ese objetivo en mente, introduciremos dos conceptos cruciales de la teoría de la complejidad computacional, los cuales nos permitirán 
establecer relaciones de \textit{dificultad computacional} entre problemas de decisión.
...
Luego, para continuar con la tarea de demostrar \textit{hardness} para $\coNP$, queremos ver que para cada $L \in \coNP$ ocurre que 
...
\begin{teorema}\label{thm:dnftaut-conp-complete}
    $\DNFTAUT$ es \coNPComplete.%$\coNPComplete$.
\end{teorema}
...
\begin{definicion}\label{def:karp-reduction}
    Definimos $f : \DNFTAUT \to \CBisim$ como el siguiente mapeo de instancias de $\DNFTAUT$ a instancias de $\CBisim$.
    % \begin{center}

    Sea $\varphi = \varphi_1 \vee ... \vee \varphi_m$ una fórmula proposicional en forma disyuntiva normal en la cual aparecen las variables 
    proposicionales $p_1,...,p_n$, definimos $f(\varphi) = \tup{\model,\model',Z}$ donde:
    % \end{center}\textsc{}
    \begin{itemize}
    ...
    \end{itemize}
    Notar que decimos que $p_i$ aparece en forma \textit{positiva} en $\varphi_j$ si $\varphi_j = ... \wedge p_i \wedge...$
    A su vez, decimos que $p_i$ aparece en forma \textit{negativa} en $\varphi_j$ si $\varphi_j = ...\wedge \neg p_i \wedge...$
...
\begin{ejemplo}
    ...
    \Cref{def:karp-reduction} construye el modelo $\model$ presentado en la \Cref{fig:karp-reduction}.
    ...
    \begin{figure}[t]
        ...
        \begin{tikzpicture}[scale=1.3]
            ...
            \node at ($(p1)+(0.35,-0.3)$)  {$p_1$};
            \node at ($(p2)+(0.35,-0.3)$) {$p_2$};
    ...
    definidas por $\R_{test}$ y la clase $\{test\}$ no es parte de la relación de indistinguibilidad del agente $agt$.
\end{ejemplo}
...
$\overrightarrow{a}$ en la que asignamos \textit{true} a las variables dentro de $U$ y \textit{false} a las variables dentro de 
...
verificando a la vez el valor obtenido por la evaluación de $\varphi$ en la asignación $\overrightarrow{a}$ asociada a $U$.

Luego, la idea es que $\varphi$ evaluará a \textit{true} en la asignación $\overrightarrow{a}$ siempre que exista un término $\varphi_j$ que permita 
...
\begin{lema}\label{lema:dnf-leqp-cbisim}
    $f$ es una reducción de Karp de $\DNFTAUT$ a $\CBisim$. Luego, se cumple que $\DNFTAUT \leq_p \CBisim$.
\end{lema}

\begin{demostracion}
    Queremos demostrar que $f$ es una reducción de Karp de $\DNFTAUT$ a $\CBisim$. Para ello, tenemos que probar que $f$ es computable 
    en tiempo polinomial y que para cada $\varphi$ fórmula proposicional en forma disyuntiva normal, $\varphi \in \DNFTAUT$ si y sólo 
    si $f(\varphi) \in \CBisim$.
    ...
    si $\tup{\modults,\modults',Z} \in \CBisim$, con $\tup{\model,\model',Z}$ definidos como en la \Cref{def:karp-reduction}.
    ...
    que atestigua dicha situación \textbf{no} es aquella que asigna \textit{false} a todas las variables proposicionales de $\varphi$. 
    Notar que esta consideración no quita generalidad, dado que no es difícil modificar el mapeo $f$ para que en dicho caso se 
    mapee $\varphi$ a dos \ultss que sean trivialmente no bisimilares. Más aún, verificar si se da esa situación es realizable en 
    tiempo polinomial debido a que model-checking para lógica proposicional está en \Poly.
    
    \bm{$\Rightarrow)$} Sea $\varphi \in \DNFTAUT$, donde $\varphi = \varphi_1 \vee ... \vee \varphi_m$ y $p_1,...,p_n$ 
    ...
    Sea $U, T \subseteq \W$ tal que $U$ satisface que para cada $s\in\rho_\model$ se cumple que $s \cap U = \emptyset$ o $s\subseteq U$ y $U \ultsExecAgagt T$, queremos ver que existe 
    ...
    Consideremos ahora la asignación $\overrightarrow{a}$ que asigna \textit{true} a las variables en $U$ y \textit{false} a las variables 
    ...
    Finalmente, $T' = X$ satisface que $Z(U) \ultsExecAgagt T'$ y, a su vez, 
    $T' \subseteq U \cup \{e\} \subseteq T = Z(T)$.
    ...
    por lo que $\tup{\modults,\modults',Z} \in \CBisim$. Así, concluímos la demostración de \bm{$\Rightarrow)$}.

    \bm{$\Leftarrow)$} Para demostrar este caso, veremos que siendo $\varphi \notin \DNFTAUT$, 
    ...
    cada $\varphi_j$. Denotemos a $U$ como el conjunto de variables a las que se le asigna \textit{true} en $\overrightarrow{a}$.
    ...
    de variables proposicionales a las cuales se les asigna \textit{true}
    ...
    Analizamos entonces todos los planes disponibles para el agente $agt$ en $\model'$ y ninguno permite encontrar un 
    $T'$ que cumpla con lo requerido, luego $Z$ no satisface ($\khi$-zig).
    Hemos demostrado que $Z$ no es una \KHilogic-bisimulación,
    lo cual demuestra que $\tup{\modults,\modults',Z} \not \in \CBisim$, con lo que concluímos la demostración de \bm{$\Leftarrow)$} 
    
    Finalmente, demostramos que $f$ es una reducción de Karp de $\DNFTAUT$ a $\CBisim$. Luego, $\DNFTAUT \leq_p \CBisim$.
\end{demostracion}
...
\begin{demostracion}
    Este corolario es consecuencia directa del \Cref{thm:dnftaut-conp-complete} y del \Cref{lema:dnf-leqp-cbisim}.
\end{demostracion}
...
\begin{demostracion}
    Este teorema es consecuencia directa de los resultados presentados en el \Cref{lema:cbisim-in-conp} y en el \Cref{col:cbisim-conphard}. 
\end{demostracion}
...
del mismo es que tiene una naturaleza, a priori, de \textit{búsqueda y verificación}, pues
...
A su vez, esta \textit{búsqueda y verificación} parece ser costosa, pues hay una cantidad exponencial de relaciones binarias
...
Una primera aproximación que surge de estas consideraciones es que $\KHiBisim \in \NP^{\coNP}$. Pues, basta con dar una 
...
\begin{lema}\label{lema:bisim-existence}
    ...
    Entonces, existe $Z \subseteq \W \times \W'$ una \KHilogic-bisimulación entre ellos si y sólo si
...
\begin{demostracion}
    ...
    Notar que la dirección \bm{$\Leftarrow)$} es directa. Demostraremos entonces \bm{$\Rightarrow)$}.
    Queremos ver que $\universalbis$ es una \KHilogic-bisimulación entre $\modults$ y $\modults'$
    dado que existe $Z \subseteq \W \times \W'$, una \KHilogic-bisimulación entre ellos.
    ...
    A su vez, como mencionamos, $\universalbis$ contiene todos los pares que satisfacen (Atom).
    Demostremos entonces que $\universalbis$ cumple ($\khi$-zig) y ($\khi$-zag).

    \begin{itemize}
        \item ($\khi$-zig) Sean $U \subseteq \W$ tal que para cada $s \in \rho_{\modults}$ se cumple que 
        $s \cap U = \emptyset$ o $s \subseteq U$, y sea $T\subseteq\W$ tal que $U \ultsExecAgi T$ queremos ver que existe 
        $T' \subseteq \W'$ tal que:
        ...
        Como $Z$ cumple (A-zag), existe $v \in \W$ tal que $(v,w') \in Z$, y como $Z$ cumple 
        ...
        Lo cual demuestra que $Z(U) = \universalbis(U)$. Por lo que $\universalbis(U) \ultsExecAgi T''$. Finalmente, concluimos 
        que $\universalbis$ satisface ($\khi$-zig).

        \item ($\khi$-zag) Análogo a ($\khi$-zig).
    \end{itemize}
    Queda demostrado que $\universalbis$ es una \KHilogic-bisimulación, lo que demuestra \bm{$\Rightarrow)$}.
\end{demostracion}

Esta caracterización de la existencia de bisimulaciones entre \ultss
nos permite profundizar nuestro primer análisis. Ahora, podemos considerar a $\KHiBisim$ como un problema de \textit{verificación}, 
...
que la parte de \textit{búsqueda} de este problema se trivializa.
...
A partir del \Cref{lema:cbisim-in-conp}, obtenemos la pertenencia a $\coNP$ de $\KHiBisim$.
...
\begin{demostracion}
    Por el \Cref{lema:bisim-existence}, sabemos que basta con verificar que $\universalbis$ sea una \KHilogic-bisimulación para decidir existencia. 
...
\begin{demostracion}
    ...
    Esto se debe al hecho de que los modelos construidos en la \Cref{def:karp-reduction} sólo cuentan con una única posible bisimulación, 
    que es la relación binaria presentada en esa misma definición. Luego, la demostración del \Cref{lema:dnf-leqp-cbisim} aplica también 
    a este caso debido a que la existencia de una \KHilogic-bisimulación entre los modelos construídos por la reducción está sujeta 
    al hecho de que la relación binaria $Z$, construída por la misma reducción, sea efectivamente una bisimulación.

    Finalmente, como $\DNFTAUT \leq_p \KHiBisim$, por el \Cref{thm:dnftaut-conp-complete} obtenemos que $\KHiBisim$ es \coNPHard.
\end{demostracion}

Juntando ambos lemas, conseguimos el siguiente resultado de complejidad computacional para $\KHiBisim$.

\begin{teorema}\label{thm:khibisim-conpcomplete}
    $\KHiBisim$ es \coNPComplete.%$\coNPComplete$.
\end{teorema}

\begin{demostracion}
    El teorema es consecuencia directa del \Cref{lema:khibisim-in-conp} y del \Cref{lema:khi-bisim-conphard}.
\end{demostracion}
...
%     \begin{itemize}
%         \item En el caso de que $\varphi$ evalúe a \textit{false} en la asignación que asigna a $p_1,...,p_n$ el valor \textit{false}
...
%         \item En el caso de que $\varphi$ evalúe a \textit{true} en la asignación que asigna a $p_1,...,p_n$ el valor \textit{false}
...
%     estamos considerando la asignación que asigna \textit{true} a las variables dentro de $U$ y asigna \textit{false} a las de $\{p_1,...,p_n\}/U$. 
...
En esta sección estudiaremos otro problema que surge naturalmente de $\KHiBisim$: dados $\modults,w$ y $\modults',w'$ 
...
Demostraremos equivalencia vía reducción de Karp entre $\KHiBisim$ y $\PKHiBisim$, es decir, demostraremos que 
...
\begin{demostracion}
    Dados $\modults,w$ y $\modults',w'$ dos \ultss punteados, por el \Cref{lema:bisim-existence} sabemos que existe 
    \KHilogic-bisimulación entre $\modults$ y $\modults'$ si y sólo si $\universalbis = \{(v,v') \mid \V(v) = \V'(v')\}$ es una 
    \KHilogic-bisimulación. 
    
    Luego una reducción $f$ que mapee $\tup{(\modults,w),(\modults',w')}$ a $\tup{\modults,\modults'}$ en el caso que $\V(w) = \V'(w')$ 
    y a $\tup{\modults*,\modults*'}$ con $\modults*$ y $\modults*'$ dos \ultss fijos no bisimilares en caso contrario, es suficiente 
...
\begin{demostracion}
    Dados $\modults$ y $\modults'$ dos \ultss, por el \Cref{lema:bisim-existence} sabemos que existe 
    \KHilogic-bisimulación entre ellos si y sólo si $\universalbis = \{(v,v') \mid \V(v) = \V'(v')\}$ es una \KHilogic-bisimulación.

    Consideremos una reducción $f$ que mapee $\tup{\modults,\modults'}$ a $\tup{(\modults,w),(\modults',w')}$ con $w,w'$ nodos de los dominios de $\modults$ 
    ...
    par de nodos $w,w'$ de cada dominio. Es posible ver que dicha reducción es suficiente para demostrar que $\KHiBisim \leq_p \PKHiBisim$.
\end{demostracion}

\begin{teorema}
    $\PKHiBisim$ es \coNPComplete.%$\coNPComplete$.
\end{teorema}

\begin{demostracion}
    Por el \Cref{lema:pointed-to-general}, tenemos que $\PKHiBisim \leq_p \KHiBisim$. Luego, como $\KHiBisim \in \coNP$ se puede ver que 
    ...
    Por el \Cref{thm:khibisim-conpcomplete}, sabemos que $\KHiBisim$ es \coNPComplete. Luego, por el \Cref{lema:general-to-pointed}, tenemos que 
    $\KHiBisim \leq_p \PKHiBisim$, lo que nos dice que $\PKHiBisim$ es \coNPHard. Finalmente, concluímos que $\PKHiBisim$ es \coNPComplete.
\end{demostracion}


Hemos demostrado entonces que $\PKHiBisim$ es \coNPComplete.
Notemos que por el \Cref{thm:new-bisim-finite-equiv}, sabemos que es equivalente decidir $\modults,w \bisim \modults',w'$ y 
$\modults,w \modequiv \modults',w'$, pues algorítmicamente siempre trabajamos con \ultss finitos. Luego, al caracterizar la complejidad 
computacional de $\PKHiBisim$, también hemos caracterizado 
...
Para terminar este capítulo, presentaremos un resultado que establece que para cada par de \ultss punteados finitos que no son bisimilares 
existe una \KHilogic-fórmula de tamaño polinomial que los distingue. Más aún, explicaremos cómo modificar \textsc{CounterexampleChecker} 
para que compute dicha fórmula en caso de existir.

Notar que dados dos \ultss punteados no bisimilares, la relación $\universalbis$ \textbf{no} es una \KHilogic-bisimulación entre ellos. 
Luego, analizando las condiciones de la \Cref{def:bisim-redefinition} en las que falla $\universalbis$, veremos que es posible construir 
la fórmula en cuestión.

\begin{teorema}
    Sean $\model,w$ y $\model',w'$ dos \ultss punteados finitos tales que $\model,w \not\bisim \model',w'$, entonces existe una 
    \KHilogic-fórmula $\varphi$ de tamaño polinomial con respecto a $|\model|+|\model'|$ tal que $\model,w \models \varphi$ y 
    $\model',w' \not\models \varphi$.
\end{teorema}

Antes de continuar con la demostración, introduciremos una notación que nos será de utilidad para construir la fórmula deseada.

Sean $\model=\tup{\W,\R,\cset{\S_i}_{i \in \AGT},\V,\ACT}$ y $\model'=\tup{\W',\R',\cset{\S_i'}_{i \in \AGT},\V',\ACT'}$ dos \ultss finitos.
Consideremos $\PROP' = \{ p \in \PROP \mid $ existe $w\in\W $ tal que $p \in \V(w)$ o existe $ w'\in\W'$ tal que $p\in\V'(w')\}$. 
Definimos para cada $s \in \rho_\model$ y $X \subseteq \W$:
\begin{itemize}
    \item $\varphi_{s} \eqdef \bigwedge\limits_{p \in \V(s)} p \wedge \bigwedge\limits_{p \in \PROP'\setminus\V(s)}\neg p$ \\
    \medskip
    \item $\varphi_{X} \eqdef \bigvee\limits_{s \in \rho_\model, s \subseteq X} \varphi_s$
\end{itemize}
Análogamente, se definen $\varphi_{s'}$ y $\varphi_{X'}$ para cada $s' \in \rho_{\model'}$ y $X' \subseteq \W'$.

A partir de la definición de $\varphi_s$, no es difícil notar que $\truthset{\model}{\varphi_s} = s$. Por otro lado, para $X \subseteq \W$
en el que para cada $s \in \rho_\model$ se cumple que $s \cap X = \emptyset$ o $s \subseteq X$, se da que $\truthset{\model}{\varphi_X} = X$.
Más aún, analizando la definición de $\universalbis$, se puede ver que $\truthset{\model'}{\varphi_s} = \universalbis(s)$ y 
$\truthset{\model'}{\varphi_X} = \universalbis(X)$ para $X$ que satisface la condición recién mencionada.

\begin{demostracion}
Sean $\model,w$ y $\model',w'$ dos \ultss punteados finitos tales que $\model,w\not\bisim\model',w'$, con 
$\model=\tup{\W,\R,\cset{\S_i}_{i \in \AGT},\V,\ACT}$ 
y $\model'=\tup{\W',\R',\cset{\S_i'}_{i \in \AGT},\V',\ACT'}$. Como se mencionó anteriormente, la estrategia para construir la fórmula 
será analizar qué condición de la \Cref{def:bisim-redefinition} no es satisfecha por $\universalbis$.

Primero, notar que si $(w,w') \not\in \universalbis$ entonces $\V(w) \neq \V'(w')$. Luego, $\varphi = \varphi_{[w]}$ cumple que 
$\model,w \models \varphi$ y $\model',w'\not\models\varphi$.
Ahora, consideremos las condiciones de la \Cref{def:bisim-redefinition} que podrían no ser satisfechas por $\universalbis$. Cabe destacar
que por su definición, es claro que $\universalbis$ siempre satisface (Atom):
\begin{itemize}
    \item (A-zig) Si $\universalbis$ no satisface (A-zig), entonces existe $v\in\W$ para el cual no existe $v'\in\W'$ tal que $(v,v')\in\universalbis$.
    Lo que nos dice que no existe $v'\in\W'$ tal que $\V(v) = \V'(v')$.
    Luego, $\varphi = \E\varphi_{[v]}$ sirve para demostrar la propiedad deseada.
    \item ($\khi$-zig) Si $\universalbis$ no satisface ($\khi$-zig) entonces existen $U,T \subseteq \W$ tales que para cada $s\in\rho_\model$
    se cumple que $s \cap U = \emptyset$ o $s\subseteq U$, y $U \ultsExecAgi T$ para los cuales no existe $T'\subseteq\W'$ tal que 
    \begin{multicols}{2}
        \begin{itemize}
            \item $\universalbis(U) \ultsExecAgi T'$, 
            \item $T' \subseteq \universalbis(T)$.
    ...
    Veamos ahora que $\varphi = \khi(\varphi_U,\varphi_{T^+})$ con $T^+ = \bigcup\limits_{w\in T}[w]$ nos sirve para demostrar la propiedad.
    
    Dado que $U \ultsExecAgi T$, entonces existe $\pi\in\S_i$ tal que $U \subseteq \sexec^\model(\pi)$ y $\R_\pi(U) \subseteq T$. Luego, como 
    $\truthset{\model}{\varphi_U} = U$ y $T \subseteq \truthset{\model}{\varphi_{T^+}} = T^+$, entonces se da que $\model,w\models\varphi$.

    Veamos ahora que $\model',w'\not\models\varphi$. Supongamos que $\model',w'\models\varphi$ y veamos que esto lleva a una contradicción. 
    Como $\model',w'\models\varphi$, entonces existe $\pi'\in\S'_i$ tal que $\truthset{\model'}{\varphi_U} \subseteq \sexec^{\model'}(\pi')$
    y $\R'_{\pi'}(\truthset{\model'}{\varphi_U}) \subseteq \truthset{\model'}{\varphi_{T^+}}$.    
    Como se mencionó antes de empezar con la demostración, tenemos que $\truthset{\model'}{\varphi_U} = \universalbis(U)$ y 
    $\truthset{\model'}{\varphi_{T^+}} = \universalbis(T^+)$. Luego, $\pi'$ atestigua que 
    $\universalbis(U) \ultsExecAgi \universalbis(T^+)$.

    Demostremos ahora que $\universalbis(T^+) \subseteq \universalbis(T)$. Sea $v'\in\universalbis(T^+)$, existe $v\in T^+$ tal que $(v,v')\in\universalbis$
    y, por lo tanto, $\V(v) = \V'(v')$.
    Pero, veamos que como $v\in T^+$, entonces existe $u\in T$ tal que $v \in [u]$, por lo que $\V(v) = \V(u)$. Luego, tenemos que 
    $\V(u) = \V'(v')$, por lo que $(u,v') \in \universalbis$. Con lo que podemos concluir que $v' \in \universalbis(T)$ y, consecuentemente, 
    $\universalbis(T^+) \subseteq \universalbis(T)$.

    Pero notemos entonces que $\universalbis(U) \ultsExecAgi \universalbis(T^+)$ y $\universalbis(T^+) \subseteq \universalbis(T)$. Luego, 
    tomando $T' = \universalbis(T^+)$ encontramos un $T'$ que atestigua que la condición ($\khi$-zig) es satisfecha para los conjuntos 
    $U$ y $T$. 
    Esto es una contradicción, dado que iniciamos este caso suponiendo que no existía tal $T'$. Luego, podemos concluir que $\model',w'\not\models\varphi$.
    \item Los casos (zag) son análogos a sus respectivos casos (zig), con la particularidad de que deben negarse las fórmulas construídas.
\end{itemize}
Es claro que la fórmula producida en cada uno de los casos analizados tiene tamaño polinomial con respecto a $|\model|+|\model'|$.
\end{demostracion}

Por último, veamos que es posible modificar $\textsc{CounterexampleChecker}$ para que, almacenando la información necesaria, construya 
las fórmulas propuestas en caso de detectar que los \ultss punteados recibidos no son bisimilares. En particular, 
si falla alguno de los chequeos de las líneas \ref{line:A-zig} o \ref{line:A-zag} es posible construir la fórmula presentada en la 
demostración para los casos (A-zig)/(A-zag) almacenando el elemento para el cual falla dicha condición. 
Por otro lado, si la ejecución de \textsc{NotSimulates} devuelve 1, es posible construir la fórmula a partir del conjunto $U$ recibido 
como input y el conjunto $T$ definido en la línea \ref{line:T-reachable-states-counterexcheck} para el cual no se encontró un testigo, 
siguiendo la estrategia presentada para los casos ($\khi$-zig)/($\khi$-zag) en la demostración del teorema. 

De esta manera, además de estudiar las propiedades computacionales de la lógica, obtenemos una nueva aplicación de nuestros métodos para
distinguir dos modelos de forma sintáctica.