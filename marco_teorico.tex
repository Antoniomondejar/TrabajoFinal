...
inspiración del campo de ``planning'' en Inteligencia Artificial, surge la noción de ejecutabilidad fuerte.
...
\begin{definicion}[Ejecutabilidad Fuerte]
    ...
    Por último, $\sexec^{\model}$($\pi$) $\eqdef \cap_{\sigma \in \pi}$ $\sexec^{\model}$($\sigma$) es el conjunto de estados en $\W$ donde $\pi$ es fuertemente ejecutable. 
\end{definicion}
...
\begin{ejemplo}
    ...
    \begin{itemize}
        ...
        Luego $\truthset{\modults}{p} = \{w_1,w_2\} \not\subseteq \sexec(\pi)$ para cada $\pi \in \S_i$.
...
\begin{definicion}\label{def:finite-ults}
    ...
    \begin{center}
        \MFD $\eqdef \{\modults \mid \modults = \tup{\W,\R,\{\S_i\}_{i\in\AGT},\V,\ACT}$ es un \ults con $\W$ es finito$\}$
    \end{center}
