...
su disposición en un determinado estado, sino que también por la composición de las mismas. A las secuencias de acciones las 
...
\begin{definicion}
    ...
    \end{center}
    donde $p \in \PROP$ e $i \in \AGT$. Las constantes y demás operadores Booleanos son definidas de la forma usual. 
...
\begin{ejemplo}\label{ejemplo:lts}
    ...
    Su representación gráfica se introduce en \Cref{fig:lts}.
    \begin{figure}[h]
        \centering
        \begin{tikzpicture}
            \node[state] (p) {$p$};
...
\begin{ejemplo}\label{ejemplo:ults}
    ...
    de $\model=\tup{\W,\R,\{\S_i\}_{i\in\AGT},\V,\ACT}$ se introduce en \Cref{fig:ults}.
    \begin{figure}[h]
        \hspace{2.3cm}
        \begin{tikzpicture}
            \node[state] (p) {$p$};
...
Una parte crucial es determinar qué se considerará un conjunto de planes \textit{adecuado}. Como mencionamos en la presentación de este capítulo, y tomando 
inspiración del campo de \textit{automated planning} en Inteligencia Artificial, surge la noción de ejecutabilidad fuerte.
