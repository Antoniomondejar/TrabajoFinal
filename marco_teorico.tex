\chapter{Preliminares: Lógica de Knowing How Basada en Incertidumbre}\label{ch:marco-teorico}
En este capítulo introduciremos un enfoque para modelar el concepto de \textit{saber cómo}, presentado en \cite{ArecesFSV25,SaraviaPHD}, el cual surge a partir de la 
...
a partir de dicho estado se puede completar. En tal caso, diremos que el plan es \textit{fuertemente ejecutable} en tal estado.

La Lógica de \textit{Knowing How} Basada en Incertidumbre (\KHilogic) toma este enfoque como punto de partida. La principal motivación por la que 
...
\begin{ejemplo}
    Consideremos un robot moviendose en una grilla, el cual tiene a su disposición la opción de moverse a cualquiera de sus 4 casilleros vecinos; es decir,
    ...
    $\ACT$ estaría definido como $\ACT = \{L,R,U,D\}$, y $\ACT^*$ sería el conjunto de secuencias finitas sobre $\ACT$, el cual está compuesto por 
...
Procederemos a introducir los modelos que representarán el entorno en el que se encuentran los agentes y las acciones que tienen a su disposición: 
...
cuales logran capturar el comportamiento de los mismos con respecto a la lógica en cuestión. 
...
\begin{ejemplo}\label{ejemplo:bisim}
    ...
    Consideremos la relación binaria $Z = \{(w_j, w_j') \mid 1 \leq j \leq 4\}$, la cual está representada gráficamente con las líneas 
...
es \PSPACEComplete. Por otro lado, en \cite{SAT_Upper_Bound} se demostró que $\SAT \in \NP^{\NP}$ en su versión para \KHlogic, aunque no se ha encontrado todavía 
